...
RUN ~otrs/bin/otrs.CheckModules.pl --all && cpanm install Jq
...
RUN systemctl restart apache2 && systemctl enable --now apache2 && systemctl enable --now mariadb
RUN su - otrs && bin/Cron.sh start
